...
func main() {
	...
	auth.Use(middleware.EnsureUserValid(conn))
	{
		...
		auth.GET("/oracle/dashboard", h.OracleDashboard)
		auth.GET("/oracle/aggregations", h.OracleAggregations)
		auth.GET("/oracle/aggregations/:id", h.OracleAggregationDetail)
		...
		auth.GET("/chain/demo/peer/status", h.ChainDemoPeerStatus)
		auth.POST("/chain/demo/peer/stop", h.ChainDemoPeerStop)
		auth.POST("/chain/demo/peer/start", h.ChainDemoPeerStart)
		auth.POST("/chain/demo/test-anchor", h.ChainDemoTestAnchor)
